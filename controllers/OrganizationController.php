...
class OrganizationController extends Controller
{
	...
	if(isset($_POST['Organization']) && $model->load(Yii::$app->request->post()))
	{
		...
		if(!empty($_FILES['Organization']['tmp_name']['org_pic']))
		{
			$file = UploadedFile::getInstance($model,'org_pic');
			$model->org_pic_type = $file->type;
			$fp = fopen($file->tempName, 'r');
			$content = fread($fp, filesize($file->tempName));
			fclose($fp);
			if($model->org_pic_type == "image/png") 
			{
				$src_img = imagecreatefrompng($file->tempName);
				$dst_img = imagecreatetruecolor(90, 70);
				imagealphablending($dst_img, false);
			...
			if($model->org_pic_type == "image/jpg" || $model->org_pic_type == "image/jpeg") 
			{
				$src_img = imagecreatefromjpeg($file->tempName);
				$dst_img = imagecreatetruecolor(90, 70);
				imagealphablending($dst_img, false);
			...
			if($model->org_pic_type == "image/gif") 
			{
				$src_img = imagecreatefromgif($file->tempName);
				$dst_img = imagecreatetruecolor(90, 70);
				imagealphablending($dst_img, false);
